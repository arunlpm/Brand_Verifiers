...
my(@brand,@aliases,@category,@twitter,@facebook,@topic,@in_brand,@in_category);
my $xls = Spreadsheet::ParseExcel::Simple->read('cosmos_brands.xls');
  foreach my $sheet ($xls->sheets) {

     while ($sheet->has_data) {
         my @data = $sheet->next_row;
         push(@brand,$data[0]); push(@aliases,$data[1]); push(@category,$data[2]);push(@twitter,$data[3]);push(@facebook,$data[4]);push(@topic,$data[5]);
     }
 }
